...
function createDemoMessages() {
    ...
    return {
        "stream:1/topic:welcome": [
            ...
            { id: 1005, sender_id: 103, sender_full_name: "Dave Wilson", sender_email: "dave@foundry.dev", type: "stream", content: "<p>Just pushed the new supervisor integration. You can now use the <strong>AI</strong> button in topic views to invoke the Meridian orchestrator.</p>", subject: "welcome", timestamp: now - 1200, stream_id: 1, flags: [], reactions: [{ emoji_name: "tada", emoji_code: "1f389", reaction_type: "unicode_emoji", user_id: 100 }, { emoji_name: "tada", emoji_code: "1f389", reaction_type: "unicode_emoji", user_id: 101 }], avatar_url: null, display_recipient: "general" },
        ],
    };
}
/**
...
export function App(props) {
    ...
    const handleLogin = (result, email) => {
        setLoginResult(result);
        if (email)
            setLoginEmail(email);
    };
    return (<div class="h-screen w-screen flex flex-col" data-component="app-shell">
      <Show when={!autoLoginLoading()} fallback={<LoadingSplash />}>
        <Show when={loginResult()} fallback={<LoginView onLogin={(result) => {
...
function LoadingSplash() {
    return (<div class="h-full flex items-center justify-center bg-[var(--background-base)]">
...
function AppShell(props) {
    ...
    onMount(() => {
        sync.setConnected(props.loginResult.org_id, props.loginResult.queue_id, props.loginResult.subscriptions, props.loginResult.users, props.loginEmail, props.loginResult.user_id);
    ...
    createEffect(() => {
        ...
        const root = document.documentElement;
        const sizeMap = { small: "13px", normal: "14px", large: "16px" };
    ...
    createEffect(() => {
        ...
        let styleEl = document.getElementById("foundry-custom-css");
        if (css) {
            if (!styleEl) {
    ...
    return (<div class="flex flex-col h-full" data-component="app-layout">
      {/* macOS title bar drag region — provides space for traffic light buttons */}
      <div data-tauri-drag-region class="flex items-center justify-end" style={{ height: "52px", "flex-shrink": "0", background: "var(--surface-sidebar)", "padding-right": "12px" }}>
      ...
      <div class="flex flex-1 min-h-0">
        ...
        <RightSidebar show={showRightSidebar()} onClose={() => setShowRightSidebar(false)}/>
      ...
      <Show when={showSettings()}>
        <SettingsView onClose={() => setShowSettings(false)} onLogout={() => {
