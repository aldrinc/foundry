...
export function ComposeBox(props) {
    ...
    const handleSend = async () => {
        ...
        setError("");
        try {
            const result = await commands.sendMessage(org.orgId, target.msgType, target.to, text, target.topic);
    ...
    return (<div class="border-t border-[var(--border-default)] bg-[var(--background-surface)] px-4 py-3" data-component="compose-box">
      <div class="flex gap-2 items-end">
        <textarea ref={textareaRef} class="flex-1 px-3 py-2 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-input)] text-[var(--text-primary)] text-sm resize-none focus:outline-none focus:border-[var(--interactive-primary)] transition-colors" style={{ "min-height": "38px", "max-height": "180px" }} placeholder={placeholder()} value={content()} onInput={(e) => handleInput(e.currentTarget.value)} onKeyDown={handleKeyDown} disabled={sending()} rows={1}/>

        <button class="shrink-0 px-3 py-2 rounded-[var(--radius-md)] bg-[var(--interactive-primary)] text-[var(--interactive-primary-text)] text-sm font-medium hover:bg-[var(--interactive-primary-hover)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed" onClick={() => void handleSend()} disabled={sending() || !content().trim()} title="Send (Cmd/Ctrl+Enter)">
          {sending() ? "..." : "Send"}
        </button>
      </div>
