...
export function HelpMenu(props) {
    ...
    return (<div class="relative" ref={menuRef}>
      <button class="p-1.5 rounded-[var(--radius-sm)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)] transition-colors" onClick={toggle} title="Help">
      ...
      <Show when={open()}>
        <div class="absolute right-0 top-full mt-1 w-[200px] bg-[var(--background-surface)] border border-[var(--border-default)] rounded-[var(--radius-md)] shadow-lg z-50 py-1">
          <MenuItem label="Keyboard shortcuts" shortcut="?" onClick={() => handleAction(props.onShowShortcuts)}/>
          <MenuItem label="Message formatting" onClick={() => handleAction(props.onShowShortcuts)}/>
