...
export function MessageList(props) {
    ...
    const narrowFilters = () => {
        const filters = nav.narrowToFilters(props.narrow);
        return filters.map(f => {
            if (f.operator === "stream" && typeof f.operand === "string") {
                const id = parseInt(f.operand, 10);
                if (!isNaN(id)) {
    ...
    const fetchMessages = async () => {
        ...
        setError("");
        try {
            const result = await sync.ensureMessages(props.narrow, narrowFilters(), { limit: 50, markRead: true });
    ...
    const fetchOlderMessages = async () => {
        ...
        setLoading(true);
        try {
            const result = await commands.getMessages(org.orgId, narrowFilters(), String(msgs[0].id), 50, 0);
            if (result.status === "ok") {
    ...
    return (<div class="flex-1 flex flex-col min-h-0" data-component="message-list">
      {/* Header */}
      <header class="h-12 flex items-center gap-2 px-4 border-b border-[var(--border-default)] bg-[var(--background-surface)] shrink-0">
        <Show when={headerColor()}>
          {(color) => (<span class="w-2.5 h-2.5 rounded-full shrink-0" style={{ "background-color": color() }}/>)}
        </Show>
        ...
        <div class="flex items-center gap-1 shrink-0">
          ...
          <Show when={nav.parseNarrow(props.narrow)?.type === "topic" || nav.parseNarrow(props.narrow)?.type === "stream"}>
            ...
            return (<button class={`p-1.5 rounded-[var(--radius-sm)] transition-colors ${supervisor.store.active
                    ...
                    : isTopic()
                        ...
                        : "text-[var(--text-quaternary)] cursor-default"}`} onClick={() => {
                    if (!isTopic())
                        return;
          ...
          <Show when={props.onToggleUserPanel}>
            <button class="p-1.5 rounded-[var(--radius-sm)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)] transition-colors" onClick={() => props.onToggleUserPanel?.()} title="Toggle user panel">
