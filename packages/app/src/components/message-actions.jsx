...
export function MessageActions(props) {
    ...
    const handleReaction = async (emojiName, emojiCode) => {
        setShowEmoji(false);
    ...
    return (<div class="absolute -top-3 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-0.5 bg-[var(--background-surface)] border border-[var(--border-default)] rounded-[var(--radius-md)] shadow-sm px-1 py-0.5" data-component="message-actions">
      {/* React button */}
      <div class="relative">
        <ActionButton title="Add reaction" onClick={() => setShowEmoji(e => !e)}>
      ...
      {/* Star toggle */}
      <ActionButton title={isStarred() ? "Unstar message" : "Star message"} onClick={handleToggleStar} active={isStarred()}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill={isStarred() ? "currentColor" : "none"}>
          <path d="M7 1.5l1.5 3 3.3.5-2.4 2.3.6 3.2L7 8.9 3.9 10.5l.6-3.2-2.4-2.3 3.4-.5L7 1.5z" stroke="currentColor" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </ActionButton>
      ...
      <ActionButton title={copied() ? "Copied!" : "Copy link"} onClick={handleCopyLink}>
        <Show when={!copied()} fallback={<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <Show when={isOwnMessage()}>
        <ActionButton title={confirming() ? "Click again to confirm" : "Delete"} onClick={handleDelete} danger={confirming()}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
function ActionButton(props) {
    return (<button onClick={(e) => { e.stopPropagation(); props.onClick(); }} class={`p-1 rounded-[var(--radius-sm)] transition-colors ${props.danger
            ? "text-[var(--status-error)] hover:bg-[var(--status-error)]/10"
            : props.active
                ? "text-amber-400 hover:bg-amber-400/10"
                : "text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)]"}`} title={props.title}>
      {props.children}
    </button>);
}
