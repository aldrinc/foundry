...
export function EmojiPicker(props: {
  ...
  const filtered = createMemo(() => {
    ...
    if (!q) return null; // Show categories when not searching
    ...
    >
      ...
      <div class="p-2 border-b border-[var(--border-default)]">
        <input
          ...
          class="w-full px-2 py-1 text-xs rounded border border-[var(--border-default)] bg-[var(--surface-input)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--interactive-primary)]"
          autofocus
        />
      </div>
                ...
                <For each={results()}>
                  {(emoji) => (
                    <button
                      ...
                      class="w-8 h-8 flex items-center justify-center rounded hover:bg-[var(--background-elevated)] transition-colors text-base"
                      title={`:${emoji.name}:`}
                    >
                      {emoji.char}
                    </button>
