...
import {
  DEFAULT_MESSAGE_QUICK_REACTIONS,
  getQuickReactionEntriesByName,
} from "./emoji-quick-reactions"
describe("emoji picker quick reactions", () => {
  test("uses the requested default message reactions in order", () => {
    expect(
      DEFAULT_MESSAGE_QUICK_REACTIONS.map(({ name, code, char }) => ({ name, code, char })),
    ).toEqual([
