...
export function SearchBar(props) {
    ...
    const doSearch = async (q) => {
        ...
        setShowResults(true);
        try {
            const result = await commands.getMessages(org.orgId, [{ operator: "search", operand: q }], "newest", 20, 0);
            if (result.status === "ok") {
    ...
    return (<div class="relative" data-component="search-bar">
      <input ref={inputRef} type="text" placeholder="Search messages..." value={query()} onInput={(e) => handleInput(e.currentTarget.value)} onFocus={() => { if (query().trim())
        setShowResults(true); }} onKeyDown={handleKeyDown} autofocus class="w-full px-3 py-1.5 text-xs rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-input)] text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--interactive-primary)] transition-colors"/>
      ...
      <Show when={showResults()}>
        <div class="absolute top-full left-0 right-0 mt-1 bg-[var(--background-surface)] border border-[var(--border-default)] rounded-[var(--radius-md)] shadow-md max-h-[400px] overflow-y-auto z-50">
          ...
          <For each={results()}>
            ...
            return (<button class="w-full text-left px-3 py-2 hover:bg-[var(--background-elevated)] transition-colors border-b border-[var(--border-default)] last:border-b-0" onClick={() => handleResultClick(msg)}>
                  <div class="flex items-center gap-1.5 mb-0.5">
                    <Show when={stream()}>
                      {(s) => (<span class="w-1.5 h-1.5 rounded-full shrink-0" style={{ "background-color": s().color || "var(--text-tertiary)" }}/>)}
                    </Show>
                  ...
                  <div class="text-xs text-[var(--text-primary)] truncate" innerHTML={msg.content}/>
                </button>);
        }}
          </For>
        </div>
