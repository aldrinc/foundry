import type { JsonValue, SupervisorEvent } from "@zulip/desktop/bindings";
...
function ToolEvent(props: { event: SupervisorEvent; type: "call" | "result" }) {
  const payload = () =>
    (props.event.payload || {}) as Record<string, JsonValue>;
  const toolName = () =>
    extractString(payload(), "tool") ||
          ...
          <For each={tasks()}>
            {(task) => (
              <div class="flex items-center gap-1.5 text-[10px] text-[var(--text-secondary)]">
                ...
                <span class="truncate">
                  {extractString(task, "title") || "Task"}
                </span>
                <span class="text-[var(--text-tertiary)]">
                  {extractString(task, "assigned_role") || ""}
                </span>
              </div>
            )}
          </For>
...
function PlanDraftEvent(props: { event: SupervisorEvent }) {
  return (
    <div class="mb-2 rounded-lg border border-[var(--status-info)]/30 bg-[var(--status-info)]/5 p-2">
      <div class="text-xs font-medium text-[var(--status-info)] mb-1">
        Plan Draft
      </div>
      <div
...
function renderMarkdown(text: string): string {
  ...
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="bg-[var(--background-base)] rounded px-1 py-0.5 text-[11px] font-mono">$1</code>',
  );
  ...
  html = html.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    '<a href="$2" class="text-[var(--interactive-primary)] underline" target="_blank" rel="noopener">$1</a>',
  );
