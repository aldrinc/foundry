...
export function EventItem(props) {
    return (<Switch fallback={<MessageEvent event={props.event}/>}>
...
// ── Message bubble ──
function MessageEvent(props) {
    const isUser = () => props.event.role === "user";
    const isLocal = () => props.event.id < 0;
    return (<div class={`mb-2 ${isUser() ? "flex justify-end" : ""}`}>
      <div class={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${isUser()
            ? "bg-[var(--interactive-primary)] text-[var(--interactive-primary-text)]"
            : "bg-[var(--background-elevated)] text-[var(--text-primary)]"} ${isLocal() ? "opacity-60" : ""}`}>
        ...
        <div class="supervisor-content whitespace-pre-wrap break-words" data-component="message-content" innerHTML={renderMarkdown(props.event.content_md || "")}/>
      </div>
    </div>);
}
// ── Thinking event (collapsible) ──
function ThinkingEvent(props) {
    return (<details class="mb-2 group">
      <summary class="flex items-center gap-1.5 cursor-pointer text-xs text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] list-none">
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" class="transition-transform group-open:rotate-90">
...
function ToolEvent(props) {
    ...
    const statusIcon = () => {
        ...
        if (err)
            return "✗";
        if (/success|complete|done/i.test(content))
            return "✓";
        return "→";
    };
    const statusColor = () => {
...
function DispatchEvent(props) {
    ...
    return (<div class="mb-2 rounded-lg border border-[var(--border-default)] bg-[var(--background-elevated)] p-2">
      ...
      <Show when={tasks().length > 0}>
        <div class="space-y-1">
          {tasks().map(task => (<div class="flex items-center gap-1.5 text-[10px] text-[var(--text-secondary)]">
...
// ── Plan draft event ──
function PlanDraftEvent(props) {
    return (<div class="mb-2 rounded-lg border border-[var(--status-info)]/30 bg-[var(--status-info)]/5 p-2">
      <div class="text-xs font-medium text-[var(--status-info)] mb-1">Plan Draft</div>
      <div class="text-xs text-[var(--text-primary)] whitespace-pre-wrap" data-component="message-content" innerHTML={renderMarkdown(props.event.content_md || "")}/>
    </div>);
}
// ── Trace fields (engine, model, etc.) ──
function TraceFields(props) {
    const fields = () => {
    ...
    return (<Show when={fields().length > 0}>
