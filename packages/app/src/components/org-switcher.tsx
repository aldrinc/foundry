...
export function OrgSwitcher(props: {
  ...
  // Don't render if only one server
  const shouldShow = () => servers().length > 1 || servers().length === 0
          ...
          <Show
            ...
            fallback={
              <div class="w-5 h-5 rounded-[var(--radius-sm)] bg-[var(--interactive-primary)] flex items-center justify-center text-[9px] font-bold text-white shrink-0">
                {(currentServer()?.realm_name || "?").charAt(0).toUpperCase()}
              </div>
            }
          >
            <img
              src={currentServer()!.realm_icon}
              alt=""
              class="w-5 h-5 rounded-[var(--radius-sm)] shrink-0"
            />
          </Show>
          <span class="text-xs font-medium text-[var(--text-primary)] truncate flex-1">
            {currentServer()?.realm_name || "Organization"}
          </span>
