...
export function FormatToolbar(props) {
    const applyFormat = (action) => {
        ...
        requestAnimationFrame(() => {
            ...
            if (!selected) {
                ...
                textarea.setSelectionRange(start + before.length, start + before.length + action.label.toLowerCase().length);
            }
            else {
    ...
    return (<div class="flex items-center gap-0.5" data-component="format-toolbar">
      {FORMAT_ACTIONS.map(action => (<button class="p-1.5 rounded-[var(--radius-sm)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)] transition-colors" onClick={(e) => {
                e.preventDefault();
                applyFormat(action);
