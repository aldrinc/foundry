...
export function DirectMessageList() {
    ...
    const participantNames = (participants) => {
        ...
        if (others.length === 0)
            return participants[0]?.full_name ?? "Unknown";
        return others.map(u => u.full_name).join(", ");
    };
    // Get initials for avatar
    const initials = (participants) => {
        const currentId = sync.store.currentUserId;
        const other = participants.find(u => u.user_id !== currentId) ?? participants[0];
        return other?.full_name?.charAt(0).toUpperCase() ?? "?";
    };
    return (<div class="border-t border-[var(--border-default)] flex flex-col" data-component="dm-list">
      {/* DM header */}
      <div class="px-3 py-2 flex items-center justify-between">
        <button class="flex items-center gap-1 text-[10px] font-medium text-[var(--text-tertiary)] uppercase tracking-wider hover:text-[var(--text-secondary)] transition-colors" onClick={() => setCollapsed(c => !c)}>
          <svg width="8" height="8" viewBox="0 0 8 8" fill="none" class={`transition-transform ${collapsed() ? "" : "rotate-90"}`}>
            <path d="M2 1l3 3-3 3" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          Direct Messages
        </button>
        <button class="p-0.5 rounded-[var(--radius-sm)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)] transition-colors" onClick={() => setShowUserPicker(s => !s)} title="New direct message">
      ...
      <Show when={showUserPicker()}>
        <div class="px-2 pb-2">
          <input type="text" placeholder="Find a user..." value={userFilter()} onInput={(e) => setUserFilter(e.currentTarget.value)} onKeyDown={(e) => {
            if (e.key === "Escape") {
                setShowUserPicker(false);
                setUserFilter("");
            }
        }} autofocus class="w-full px-2 py-1 text-xs rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-input)] text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--interactive-primary)] transition-colors mb-1"/>
          <div class="max-h-[180px] overflow-y-auto rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--background-surface)]">
            <Show when={filteredUsers().length === 0}>
              <div class="text-[10px] text-[var(--text-tertiary)] text-center py-2">No users found</div>
            </Show>
            <For each={filteredUsers()}>
              {(user) => (<button class="w-full flex items-center gap-2 px-2 py-1.5 text-left text-xs hover:bg-[var(--background-elevated)] transition-colors" onClick={() => startDm(user)}>
      ...
      <Show when={!collapsed()}>
        <div class="overflow-y-auto max-h-[200px] pb-1">
          <Show when={dmConversations().length === 0 && !showUserPicker()}>
            <p class="text-[10px] text-[var(--text-tertiary)] px-3 py-1">No conversations yet</p>
          </Show>
          <For each={dmConversations()}>
            {(convo) => (<button class="w-full flex items-center gap-2 px-3 py-1.5 text-left text-sm transition-colors hover:bg-[var(--background-surface)]" classList={{
                "bg-[var(--background-surface)]": nav.activeNarrow() === convo.narrow,
