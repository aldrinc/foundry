...
export function StreamSidebar(props) {
    ...
    const pinnedStreams = createMemo(() => sync.store.subscriptions
        .filter(s => s.pin_to_top && !s.is_muted)
        .sort((a, b) => a.name.localeCompare(b.name)));
    const regularStreams = createMemo(() => sync.store.subscriptions
        .filter(s => !s.pin_to_top && !s.is_muted)
        .sort((a, b) => a.name.localeCompare(b.name)));
    const mutedStreams = createMemo(() => sync.store.subscriptions
        .filter(s => s.is_muted)
        .sort((a, b) => a.name.localeCompare(b.name)));
    const handleStreamClick = (streamId) => {
        nav.setActiveNarrow(`stream:${streamId}`);
    };
    return (<aside class="w-[260px] border-r border-[var(--border-default)] bg-[var(--surface-sidebar)] flex flex-col shrink-0" data-component="stream-sidebar">
      {/* User header — avatar dropdown trigger */}
      <div class="relative h-12 px-3 border-b border-[var(--border-default)] flex items-center">
        <button class="flex items-center gap-2 min-w-0 w-full rounded-[var(--radius-sm)] hover:bg-[var(--background-surface)] transition-colors px-1 py-1 -mx-1" onClick={() => setShowPersonalMenu(s => !s)}>
        ...
        <Show when={showPersonalMenu()}>
          <PersonalMenu onClose={() => setShowPersonalMenu(false)} onOpenSettings={() => props.onOpenSettings?.()} onLogout={() => props.onLogout?.()}/>
        </Show>
      </div>

      {/* Navigation views */}
      <div class="border-b border-[var(--border-default)] py-1">
        <NavButton label="Inbox" active={nav.activeNarrow() === null} onClick={() => nav.setActiveNarrow(null)} icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 4l5 3 5-3M2 4v6a1 1 0 001 1h8a1 1 0 001-1V4M2 4a1 1 0 011-1h8a1 1 0 011 1" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>}/>
        <NavButton label="Recent Topics" active={nav.activeNarrow() === "recent-topics"} onClick={() => nav.setActiveNarrow("recent-topics")} icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1v6l4 2" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/><circle cx="7" cy="7" r="6" stroke="currentColor" stroke-width="1.2"/></svg>}/>
        <NavButton label="Starred Messages" active={nav.activeNarrow() === "starred"} onClick={() => nav.setActiveNarrow("starred")} icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1l1.8 3.6L13 5.2l-3 2.9.7 4.1L7 10.3 3.3 12.2l.7-4.1-3-2.9 4.2-.6L7 1z" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>}/>
      ...
      <div class="flex-1 overflow-y-auto py-1 min-h-0">
        ...
        <Show when={pinnedStreams().length > 0}>
          <For each={pinnedStreams()}>
            {(stream) => (<StreamItem stream={stream} active={nav.activeNarrow()?.startsWith(`stream:${stream.stream_id}`) ?? false} unreadCount={sync.store.unreadCounts[stream.stream_id] ?? 0} onClick={() => handleStreamClick(stream.stream_id)}/>)}
          </For>
        </Show>
        ...
        <Show when={mutedStreams().length > 0}>
          ...
          <For each={mutedStreams()}>
            {(stream) => (<StreamItem stream={stream} active={nav.activeNarrow()?.startsWith(`stream:${stream.stream_id}`) ?? false} unreadCount={sync.store.unreadCounts[stream.stream_id] ?? 0} onClick={() => handleStreamClick(stream.stream_id)} muted/>)}
          </For>
        </Show>
      </div>
...
function StreamItem(props) {
    ...
    const handleTopicContextMenu = (e, topicName) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleMarkTopicAsRead = async (topicName) => {
        setContextMenu(null);
        try {
    ...
    return (<div>
      <button class="w-full flex items-center gap-2 px-3 py-1.5 text-left text-sm transition-colors hover:bg-[var(--background-surface)]" classList={{
        ...
        {/* Color dot */}
        <span class="w-2 h-2 rounded-full shrink-0" style={{ "background-color": props.stream.color || "var(--text-tertiary)" }}/>
        ...
        <div class="ml-6 border-l border-[var(--border-default)]">
          ...
          <Show when={topicError() && !loadingTopics()}>
            <div class="px-3 py-1 flex items-center gap-1">
              ...
              <button class="text-[10px] text-[var(--interactive-primary)] hover:underline" onClick={(e) => {
            e.stopPropagation();
            setTopicError("");
            setTopics([]);
            toggleExpand(e);
          ...
          <For each={topics()}>
            {(topic) => (<button class="w-full flex items-center gap-2 px-3 py-1 text-left text-xs text-[var(--text-secondary)] hover:bg-[var(--background-surface)] hover:text-[var(--text-primary)] transition-colors" classList={{
                "text-[var(--text-primary)] bg-[var(--background-surface)]": nav.activeNarrow() === `stream:${props.stream.stream_id}/topic:${topic.name}`,
            }} onClick={() => handleTopicClick(topic.name)} onContextMenu={(e) => handleTopicContextMenu(e, topic.name)}>
      ...
      <Show when={contextMenu()}>
        {(ctx) => (<div class="fixed z-50 bg-[var(--background-surface)] border border-[var(--border-default)] rounded-[var(--radius-md)] shadow-md py-1 min-w-[160px]" style={{ left: `${ctx().x}px`, top: `${ctx().y}px` }}>
            <Show when={ctx().type === "stream"}>
              ...
              <ContextMenuItem label={props.muted ? "Unmute channel" : "Mute channel"} onClick={handleToggleMute}/>
            </Show>

            <Show when={ctx().type === "topic" && ctx().topicName}>
              <ContextMenuItem label="Mark topic as read" onClick={() => handleMarkTopicAsRead(ctx().topicName)}/>
              <div class="my-1 border-t border-[var(--border-default)]"/>
              <ContextMenuItem label="Open in new view" onClick={() => {
                setContextMenu(null);
                nav.setActiveNarrow(`stream:${props.stream.stream_id}/topic:${ctx().topicName}`);
            }}/>
            </Show>
          </div>)}
      </Show>
...
function NavButton(props) {
    return (<button class="w-full flex items-center gap-2 px-3 py-1.5 text-left text-sm transition-colors hover:bg-[var(--background-surface)]" classList={{
