...
export function RightSidebar(props) {
    ...
    return (<Show when={props.show}>
      <aside class="w-[240px] border-l border-[var(--border-default)] bg-[var(--surface-sidebar)] flex flex-col shrink-0" data-component="right-sidebar">
        {/* Header */}
        <div class="h-12 px-3 border-b border-[var(--border-default)] flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            ...
            <button class="p-1 rounded-[var(--radius-sm)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)] transition-colors" onClick={props.onClose} title="Close panel">
        ...
        <Show when={contextUsers().length > 5}>
          <div class="px-2 py-2">
            <input type="text" placeholder="Filter users..." value={userFilter()} onInput={(e) => setUserFilter(e.currentTarget.value)} class="w-full px-2 py-1 text-xs rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-input)] text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--interactive-primary)] transition-colors"/>
          </div>
        </Show>
        ...
        <div class="flex-1 overflow-y-auto py-1">
          <For each={filteredUsers()}>
            {(user) => (<UserRow user={user} isCurrentUser={user.user_id === sync.store.currentUserId} roleLabel={roleLabel(user.role)} onSendDm={() => startDm(user)}/>)}
          </For>
...
function UserRow(props) {
    ...
    return (<div>
      <button class="w-full flex items-center gap-2 px-3 py-1.5 text-left hover:bg-[var(--background-surface)] transition-colors group" onClick={() => setExpanded(e => !e)}>
        {/* Avatar + presence */}
      ...
      <Show when={expanded()}>
        <div class="mx-3 mb-2 p-2 rounded-[var(--radius-md)] bg-[var(--background-surface)] border border-[var(--border-default)]">
          ...
          <Show when={!props.isCurrentUser}>
            <button class="w-full text-xs px-2 py-1 rounded-[var(--radius-sm)] bg-[var(--interactive-primary)] text-[var(--interactive-primary-text)] hover:bg-[var(--interactive-primary-hover)] transition-colors" onClick={(e) => {
            e.stopPropagation();
            props.onSendDm();
        }}>
              Send message
