...
export interface NavigationContext {
  ...
  narrowToFilters(
    narrow: string,
  ): { operator: string; operand: string | number[] }[];
...
export function NavigationProvider(props: { children: JSX.Element }) {
  ...
  const nav: NavigationContext = {
    ...
    narrowToFilters(narrow: string) {
      ...
      if (narrow.startsWith("stream:")) {
        ...
        if (topicSep >= 0) {
          filters.push({
            operator: "stream",
            operand: rest.slice(0, topicSep),
          });
          filters.push({
            operator: "topic",
            operand: rest.slice(topicSep + 7),
          });
        } else {
    ...
    isSpecialNarrow(narrow: string) {
      return (
        SPECIAL_NARROWS.includes(narrow as SpecialNarrow) ||
        narrow.startsWith("search:")
      );
    },
  };
