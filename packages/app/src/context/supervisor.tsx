...
const SUPERVISOR_FALLBACK_WARNING =
  "Live updates unavailable. Refreshing every 5 seconds.";
...
export function SupervisorProvider(props: {
  ...
  const [store, setStore] = createStore<SupervisorStore>({
    ...initialStore,
    seenEventIds: new Set(),
    pendingUserEchoes: new Map(),
  });
  ...
  function applySupervisorEvent(state: SupervisorStore, evt: SupervisorEvent) {
    ...
    if (evt.client_msg_id && state.pendingUserEchoes.has(evt.client_msg_id)) {
      const localId = state.pendingUserEchoes.get(evt.client_msg_id)!;
      state.events = state.events.filter((e) => e.id !== localId);
      state.pendingUserEchoes.delete(evt.client_msg_id);
    }
    ...
    if (evt.kind === "thinking") {
      state.lastThinkingPreview = evt.content_md || "";
      state.lastThinkingEventMs = Date.now();
    } else if (evt.kind === "message" && evt.role === "assistant") {
  ...
  async function refreshSidebar(topicScopeId: string) {
    if (
      sidebarPollInFlight ||
      store.topicScopeId !== topicScopeId ||
      !store.active
    )
      return;
    ...
    try {
      const result = await commands.getSupervisorSidebar(
        props.orgId,
        topicScopeId,
      );
  ...
  async function pollSession(
    ...
    try {
      const result = await commands.getSupervisorSession(
        props.orgId,
        topicScopeId,
        store.afterId,
        200,
      ...
      if (
        ...
        setStore("status", "disconnected");
        setWarning(
          error?.message || error?.toString() || "Failed to load supervisor",
          "error",
        );
      }
    } finally {
  ...
  async function setupEventListeners() {
    ...
    const fns = await Promise.all([
      listen<any>(`supervisor:${eventId}:events`, (e) =>
        handleSupervisorEvent(e.payload),
      ),
      listen<any>(`supervisor:${eventId}:session`, (e) =>
        handleSessionState(e.payload),
      ),
      listen<any>(`supervisor:${eventId}:connected`, () => handleConnected()),
      listen<any>(`supervisor:${eventId}:disconnected`, (e) =>
        handleDisconnected(e.payload),
      ),
    ]);
      ...
      setupEventListeners()
        ...
        .then((result) => {
          if (result.status === "error") {
            console.error(
              "[Supervisor] Failed to start SSE stream:",
              result.error,
            );
