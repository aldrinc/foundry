...
export function ZulipSyncProvider(props: {
  ...
  const maybeNotifyForMessage = (message: Message) => {
    const shouldNotify = shouldNotifyMessage(
      message,
      {
    ...
    const streamName = message.stream_id
      ? store.subscriptions.find(
          (subscription) => subscription.stream_id === message.stream_id,
        )?.name
      : undefined;
  ...
  const sync: ZulipSync = {
    ...
    setConnected(orgId, queueId, subscriptions, users, loginEmail, userId) {
      ...
      if (resolvedUserId) {
        ...
        console.log(
          `[ZulipSync] Current user from API: ${user?.full_name ?? "unknown"} (ID: ${resolvedUserId})`,
        );
        if (user) {
    ...
    addMessages(narrow, messages) {
      setStore(
        produce((s) => {
          s.messages[narrow] = mergeMessagesById(
            s.messages[narrow] || [],
            messages,
          );
        }),
    ...
    async ensureMessages(narrow, filters, options) {
      ...
      const request = (async () => {
        try {
          ...
          sync.setMessageLoadState(
            narrow,
            result.data.found_oldest ? "loaded-all" : "idle",
          );
          ...
          if (markRead) {
            ...
            if (unreadIds.length > 0) {
              await commands.updateMessageFlags(
                props.orgId,
                unreadIds,
                "add",
                "read",
            ...
            if (idx >= 0) {
              if (op === "add") {
                ...
                msgs[idx].reactions = (msgs[idx].reactions || []).filter(
                  (r) =>
                    !(
                      r.emoji_code === reaction.emoji_code &&
        ...
        setStore(
          produce((s) => {
            for (const sub of data.subscriptions) {
              if (
                !s.subscriptions.find((ss) => ss.stream_id === sub.stream_id)
              ) {
        ...
        setStore(
          produce((s) => {
            const removeIds = new Set(
              data.subscriptions.map((ss: any) => ss.stream_id),
            );
            s.subscriptions = s.subscriptions.filter(
              (ss) => !removeIds.has(ss.stream_id),
            );
          }),
        );
        ...
        setStore(
          produce((s) => {
            const sub = s.subscriptions.find(
              (ss) => ss.stream_id === data.stream_id,
            );
            if (sub) {
    ...
    handleDeleteMessageEvent(data: any) {
      ...
      setStore(
        produce((s) => {
          for (const narrow of Object.keys(s.messages)) {
            s.messages[narrow] = s.messages[narrow].filter(
              (m) => !deleteSet.has(m.id),
            );
          }
        }),
      );
