...
describe("shouldNotifyMessage", () => {
  test("never notifies for self-sent messages", () => {
    ...
    expect(
      shouldNotifyMessage(message, DEFAULT_PREFERENCES, {
        currentUserId: 22,
  ...
  test("respects the direct-message toggle", () => {
    ...
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, dmNotifs: true },
        {
          currentUserId: 99,
          currentUserEmail: "desdemona@example.com",
        },
    ...
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, dmNotifs: false },
        {
          currentUserId: 99,
          currentUserEmail: "desdemona@example.com",
        },
  ...
  test("respects mention preferences for direct mentions", () => {
    ...
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, mentionNotifs: true },
        {
          currentUserId: 99,
          currentUserEmail: "desdemona@example.com",
        },
    ...
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, mentionNotifs: false },
        {
          currentUserId: 99,
          currentUserEmail: "desdemona@example.com",
        },
  ...
  test("uses channel notifications for ordinary stream traffic", () => {
    ...
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, channelNotifs: false },
        context,
      ),
    ).toBe(false);
    expect(
      shouldNotifyMessage(
        message,
        { ...DEFAULT_PREFERENCES, channelNotifs: true },
        context,
      ),
    ).toBe(true);
  });
...
describe("notification formatting", () => {
  ...
  test("builds stream notification titles with channel context", () => {
    expect(buildNotificationTitle(createMessage(), "Denmark")).toBe(
      "Alicia (#Denmark > deploys)",
    );
  });
  ...
  test("strips HTML in notification bodies", () => {
    expect(buildNotificationBody("<p>Hello <strong>world</strong></p>")).toBe(
      "Hello world",
    );
  });
  ...
  test("falls back to attached-file text for media-only messages", () => {
    expect(
      buildNotificationBody('<p><img src="/user_uploads/1/file.png"></p>'),
    ).toBe("(attached file)");
  });
});
