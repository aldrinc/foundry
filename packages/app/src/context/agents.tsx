    ...
    raw
      .map((entry) => {
        ...
        return {
          ...
          scopeMode:
            item.scopeMode === "selected_streams"
              ? "selected_streams"
              : "all_topics",
...
export function AgentsProvider(props: { orgId: string; children: JSX.Element }) {
  ...
  onMount(async () => {
    try {
      ...
      if (result.status === "ok" && result.data) {
        setStore("delegates", hydrateDelegates(JSON.parse(result.data)));
      } else if (IS_DEMO) {
        setStore(
          "delegates",
          DELEGATE_TEMPLATES.map((template) => createTemplateAgent(template)),
        );
      }
    } catch {
      if (IS_DEMO) {
  ...
  const ctx: AgentsContextValue = {
    ...
    async upsertAgent(agent) {
      ...
      const exists = store.delegates.some(
        (delegate) => delegate.id === trimmedId && delegate.id !== agent.id,
      );
      if (exists) {
        return {
          ok: false,
          error: `An agent with id "${trimmedId}" already exists.`,
        };
      }
      ...
      const nextAgent: DelegateAgent = {
        ...
        preferredModel: agent.inheritProviders
          ? ""
          : agent.preferredModel.trim(),
    ...
    async deleteAgent(id) {
      const nextDelegates = store.delegates.filter(
    ...
    availableDelegatesForStream(streamId) {
      return sortDelegates(
        store.delegates.filter((delegate) =>
          delegateMatchesStream(delegate, streamId),
        ),
      );
    },
  };
