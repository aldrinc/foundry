...
import { buildSupervisorDelegateContextFromDelegates, normalizeProvider, } from "./agent-runtime";
...
export const DELEGATE_TEMPLATES = [
    {
        id: "researcher",
        name: "Researcher",
        emoji: "R",
...
function createTemplateAgent(template) {
...
function hydrateDelegates(raw) {
    if (!Array.isArray(raw))
        return [];
    return sortDelegates(raw
        .map((entry) => {
...
function agentMatchesStream(agent, streamId) {
    if (!agent.enabled || !agent.delegateEligible)
        return false;
    if (agent.scopeMode === "all_topics")
        return true;
    if (!streamId)
        return false;
...
export function AgentsProvider(props) {
    ...
    async function persistDelegates(nextDelegates) {
    ...
    const ctx = {
        ...
        buildSupervisorDelegateContext(streamId) {
            return buildSupervisorDelegateContextFromDelegates(store.delegates.filter((delegate) => agentMatchesStream(delegate, streamId)));
        },
    };
    return (<AgentsContext.Provider value={ctx}>
      {props.children}
...
export function createAgentFromTemplate(template, existingIds) {
    const base = createTemplateAgent(template);
    if (!existingIds.includes(base.id))
        return base;
    let index = 2;
