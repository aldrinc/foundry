...
export function SettingsServers() {
    ...
    return (<div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <button class="px-2.5 py-1 text-[11px] rounded-[var(--radius-sm)] bg-[var(--interactive-primary)] text-white hover:opacity-90 transition-opacity" onClick={() => setShowAdd(s => !s)}>
      ...
      <Show when={showAdd()}>
        <div class="p-3 bg-[var(--background-base)] rounded-[var(--radius-md)] border border-[var(--border-default)] space-y-3">
          <div>
            ...
            <input type="text" class="w-full text-xs bg-[var(--background-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] font-mono" placeholder="https://chat.example.com" value={newUrl()} onInput={(e) => setNewUrl(e.currentTarget.value)}/>
          </div>
          <button class="px-3 py-1.5 text-xs rounded-[var(--radius-sm)] bg-[var(--interactive-primary)] text-white hover:opacity-90 disabled:opacity-50" disabled={!newUrl().trim()}>
            Connect
          </button>
        </div>
      </Show>
      ...
      {/* Server list */}
      <Show when={servers().length > 0} fallback={<div class="text-center py-8">
        ...
        <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
          <For each={servers()}>
            {(server) => (<div class="flex items-center justify-between px-3 py-3 border-b border-[var(--border-default)] last:border-b-0">
                <div class="min-w-0">
