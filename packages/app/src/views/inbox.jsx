...
export function InboxView() {
    ...
    const totalUnread = createMemo(() => sync.store.unreadItems.reduce((sum, item) => sum + item.count, 0));
    const handleTopicClick = (streamId, topic) => {
        nav.setActiveNarrow(`stream:${streamId}/topic:${topic}`);
    };
    const handleStreamClick = (streamId) => {
        nav.setActiveNarrow(`stream:${streamId}`);
    };
    return (<div class="flex-1 flex flex-col" data-component="inbox-view">
      {/* Header */}
      ...
      <div class="flex-1 overflow-y-auto">
        <Show when={groupedUnreads().length > 0} fallback={<div class="text-center py-12">
          ...
          <For each={groupedUnreads()}>
            {(group) => (<div data-component="inbox-stream-group">
                {/* Stream header */}
                <button class="w-full flex items-center gap-2 px-4 py-2 bg-[var(--background-surface)] border-b border-[var(--border-default)] hover:bg-[var(--background-base)] transition-colors" onClick={() => handleStreamClick(group.streamId)}>
                  <span class="w-2.5 h-2.5 rounded-full shrink-0" style={{ "background-color": group.streamColor || "var(--text-tertiary)" }}/>
                ...
                <For each={group.topics.sort((a, b) => b.last_message_id - a.last_message_id)}>
                  {(item) => (<button class="w-full flex items-center gap-2 px-4 pl-9 py-1.5 hover:bg-[var(--background-surface)] transition-colors text-left" onClick={() => handleTopicClick(item.stream_id, item.topic)} data-component="inbox-topic-item">
