...
export function SettingsEmoji() {
    ...
    return (<div class="space-y-6">
      ...
      <input type="text" class="w-full text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] placeholder:text-[var(--text-quaternary)]" placeholder="Search emoji..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)}/>
      ...
      <Show when={filtered().length > 0} fallback={<div class="text-center py-8">
        ...
        <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
          <For each={filtered()}>
            {(emoji) => (<div class="flex items-center justify-between px-3 py-2 border-b border-[var(--border-default)] last:border-b-0">
