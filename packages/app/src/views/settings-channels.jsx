...
export function SettingsChannels() {
    ...
    return (<div class="space-y-6">
      ...
      {/* Search */}
      <input type="text" class="w-full text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] placeholder:text-[var(--text-quaternary)]" placeholder="Search channels..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)}/>

      {/* Channel list */}
      <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
        <Show when={filteredChannels().length > 0} fallback={<div class="text-center py-6 text-xs text-[var(--text-tertiary)]">
              No channels found
            </div>}>
          <For each={filteredChannels()}>
            {(channel) => (<div class="border-b border-[var(--border-default)] last:border-b-0">
                <div class="flex items-center justify-between px-3 py-2">
                  <div class="flex items-center gap-2 min-w-0">
                    <span class="w-2.5 h-2.5 rounded-full shrink-0" style={{ "background-color": channel.color || "var(--text-tertiary)" }}/>
                  ...
                  <button class="text-[10px] text-[var(--interactive-primary)] hover:underline shrink-0" onClick={() => setEditingId(editingId() === channel.stream_id ? null : channel.stream_id)}>
                ...
                <Show when={editingId() === channel.stream_id}>
                  <div class="px-3 pb-3 space-y-3 bg-[var(--background-base)] border-t border-[var(--border-default)]">
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <button class={`relative w-8 h-[18px] rounded-full shrink-0 transition-colors ${channel.pin_to_top ? "bg-[var(--interactive-primary)]" : "bg-[var(--border-default)]"}`}>
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <button class={`relative w-8 h-[18px] rounded-full shrink-0 transition-colors ${channel.is_muted ? "bg-[var(--interactive-primary)]" : "bg-[var(--border-default)]"}`}>
                    ...
                    <SettingRow label="Channel color" description="Customize the color dot in the sidebar">
                      <input type="color" class="w-6 h-6 rounded border border-[var(--border-default)] cursor-pointer" value={channel.color || "#76ce90"}/>
                    </SettingRow>
                  </div>
                </Show>
              </div>)}
