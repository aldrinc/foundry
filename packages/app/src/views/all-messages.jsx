...
export function AllMessagesView() {
    ...
    const fetchMessages = async () => {
        ...
        setError("");
        try {
            const result = await sync.ensureMessages("all-messages", [], { limit: 50, markRead: true });
    ...
    const fetchOlderMessages = async () => {
        ...
        setLoading(true);
        try {
            const result = await commands.getMessages(org.orgId, [], String(msgs[0].id), 50, 0);
            if (result.status === "ok") {
    ...
    const needsDivider = (msg, prev) => {
        if (!prev)
            return true;
        if (msg.stream_id !== prev.stream_id)
            return true;
        if (msg.subject !== prev.subject)
            return true;
        if (msg.type !== prev.type)
            return true;
        return false;
    };
    return (<div class="flex-1 flex flex-col min-h-0" data-component="all-messages-view">
      {/* Header */}
      <header class="h-12 flex items-center gap-2 px-4 border-b border-[var(--border-default)] bg-[var(--background-surface)] shrink-0">
        ...
        <h1 class="text-sm font-semibold text-[var(--text-primary)]">All Messages</h1>
      </header>

      {/* Messages */}
      <div ref={scrollContainer} class="flex-1 overflow-y-auto" onScroll={handleScroll}>
        ...
        <Show when={loadState() === "loaded-all" && messages().length > 0}>
          <div class="text-center py-3">
            <span class="text-xs text-[var(--text-tertiary)]">Beginning of messages</span>
          </div>
        </Show>

        <Show when={!loading() || messages().length > 0} fallback={<div class="flex-1 flex items-center justify-center py-12">
          ...
          <For each={messages()}>
            ...
            return (<>
                  <Show when={showDivider()}>
                    <button class="w-full flex items-center gap-1.5 px-4 py-1.5 text-left bg-[var(--background-surface)] border-b border-t border-[var(--border-default)] hover:bg-[var(--background-elevated)] transition-colors sticky top-0 z-10" onClick={() => {
                    const ctx = context();
                    if (ctx.narrow)
                        nav.setActiveNarrow(ctx.narrow);
                }}>
        ...
        <Show when={error()}>
          <div class="text-center py-4">
            ...
            <button class="text-xs text-[var(--interactive-primary)] mt-1 hover:underline" onClick={fetchMessages}>
              Retry
            </button>
          </div>
        </Show>
