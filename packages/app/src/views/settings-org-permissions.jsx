...
export function SettingsOrgPermissions() {
    ...
    return (<div class="space-y-6">
      ...
      <SettingToggle label="Require invitation to join" description="New users must receive an invitation to join" checked={inviteOnly()} onChange={setInviteOnly}/>
      ...
      <SettingRow label="Email domain restrictions" description="Restrict signups to specific email domains">
        <input type="text" class="text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] w-[140px]" placeholder="e.g. company.com" value={emailDomains()} onInput={(e) => setEmailDomains(e.currentTarget.value)}/>
      </SettingRow>
      ...
      <SettingRow label="Message editing" description="Policy for editing sent messages">
        <select class="text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] min-w-[140px]" value={editPolicy()} onChange={(e) => setEditPolicy(e.currentTarget.value)}>
      ...
      <SettingRow label="Message deletion" description="Policy for deleting messages">
        <select class="text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] min-w-[140px]" value={deletePolicy()} onChange={(e) => setDeletePolicy(e.currentTarget.value)}>
      ...
      <SettingToggle label="Require topics" description="Require a topic for every message sent to a channel" checked={requireTopics()} onChange={setRequireTopics}/>
    </div>);
}
function PermissionSelect(props) {
    return (<select class="text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] min-w-[140px]" value={props.value} onChange={(e) => props.onChange(e.currentTarget.value)}>
