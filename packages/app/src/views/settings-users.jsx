...
export function SettingsUsers() {
    ...
    const activeUsers = createMemo(() => sync.store.users
    ...
    const deactivatedUsers = createMemo(() => sync.store.users
        .filter(u => !u.is_active)
        .sort((a, b) => a.full_name.localeCompare(b.full_name)));
    ...
    return (<div class="space-y-6">
      ...
      {/* Search */}
      <input type="text" class="w-full text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] placeholder:text-[var(--text-quaternary)]" placeholder="Search users..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)}/>

      {/* Active users */}
      <Show when={tab() === "active"}>
        <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
          <For each={activeUsers()}>
            {(user) => (<div class="flex items-center justify-between px-3 py-2 border-b border-[var(--border-default)] last:border-b-0">
      ...
      <Show when={tab() === "deactivated"}>
        <Show when={deactivatedUsers().length > 0} fallback={<div class="text-center py-8 text-xs text-[var(--text-tertiary)]">No deactivated users</div>}>
          <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
            <For each={deactivatedUsers()}>
              {(user) => (<div class="flex items-center justify-between px-3 py-2 border-b border-[var(--border-default)] last:border-b-0 opacity-60">
                  <div class="flex items-center gap-2 min-w-0">
...
function TabBtn(props) {
    return (<button onClick={props.onClick} class={`pb-2 text-xs transition-colors flex items-center gap-1 ${props.active
