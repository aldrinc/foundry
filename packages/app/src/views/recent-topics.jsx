...
export function RecentTopicsView() {
    ...
    onMount(async () => {
        if (sync.isNarrowHydrated(ALL_MESSAGES_NARROW))
            return;
        setLoading(true);
        try {
            const result = await sync.ensureMessages(ALL_MESSAGES_NARROW, [], { limit: 200, markRead: false });
    ...
    const topics = createMemo(() => {
        ...
        for (const messages of messageSources) {
            ...
            for (const [key, groupedMessages] of topicMessages.entries()) {
                ...
                if (isNaN(streamId) || groupedMessages.length === 0)
                    continue;
        ...
        return Array.from(topicMap.values()).sort((a, b) => b.lastMessage.timestamp - a.lastMessage.timestamp);
    });
    const formatTime = (ts) => {
    ...
    return (<div class="flex-1 flex flex-col min-h-0" data-component="recent-topics-view">
      {/* Header */}
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <Show when={!loading() && topics().length > 0}>
          ...
          <For each={topics()}>
            ...
            return (<button class="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-[var(--background-surface)] transition-colors border-b border-[var(--border-default)]/50" onClick={() => nav.setActiveNarrow(`stream:${entry.streamId}/topic:${entry.topic}`)}>
                  {/* Stream color dot + name > topic */}
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5 flex-wrap">
                      <span class="w-2 h-2 rounded-full shrink-0" style={{ "background-color": entry.streamColor }}/>
                  ...
                  <div class="flex items-center shrink-0" style={{ gap: "3px" }}>
                    <For each={participants()}>
                      {(p) => (<div class="rounded-full flex items-center justify-center font-medium shrink-0" style={{
                        width: "20px",
                        height: "20px",
                        "min-width": "20px",
                        "font-size": "10px",
