...
export function SettingsAccount(props) {
    ...
    return (<div class="space-y-6">
      ...
      <SettingToggle label="Send typing notifications" description="Let others see when you are typing a message" checked={store.sendTyping} onChange={(v) => setSetting("sendTyping", v)}/>

      <SettingToggle label="Send read receipts" description="Let others see when you have read their messages" checked={store.sendReadReceipts} onChange={(v) => setSetting("sendReadReceipts", v)}/>

      <SettingToggle label="Show availability" description="Display your online/idle/offline status to other users" checked={store.showAvailability} onChange={(v) => setSetting("showAvailability", v)}/>

      <SettingRow label="Email address visibility" description="Who can see your email address">
        <select class="text-xs bg-[var(--background-base)] border border-[var(--border-default)] rounded-[var(--radius-sm)] px-2 py-1.5 text-[var(--text-primary)] min-w-[140px]" value={store.emailVisibility} onChange={(e) => setSetting("emailVisibility", e.currentTarget.value)}>
      ...
      <button onClick={props.onLogout} class="px-3 py-1.5 text-xs rounded-[var(--radius-md)] bg-[var(--status-error)] text-white hover:opacity-90 transition-opacity">
        Log out
      </button>
    </div>);
}
