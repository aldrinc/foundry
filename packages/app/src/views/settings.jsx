...
export function SettingsView(props) {
    ...
    return (<div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={props.onClose}>
      <div class="w-[720px] max-h-[85vh] bg-[var(--background-surface)] rounded-[var(--radius-lg)] shadow-lg border border-[var(--border-default)] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div class="flex items-center justify-between px-4 py-3 border-b border-[var(--border-default)] shrink-0">
          ...
          <button onClick={props.onClose} class="p-1 rounded-[var(--radius-sm)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--background-elevated)]">
            ...
            <For each={NAV}>
              {(category) => (<div class="mb-1">
                  ...
                  <For each={category.items}>
                    {(item) => (<button class={`w-full flex items-center gap-2 px-3 py-1.5 text-left text-xs transition-colors ${activeSection() === item.id
                    ? "bg-[var(--interactive-primary)]/10 text-[var(--interactive-primary)] font-medium"
                    : "text-[var(--text-secondary)] hover:bg-[var(--background-elevated)] hover:text-[var(--text-primary)]"}`} onClick={() => setActiveSection(item.id)}>
