  ...
  return (
    <div class="space-y-6">
      ...
      <p class="text-xs text-[var(--text-tertiary)]">
        Linkifiers automatically turn patterns in messages into links. For
        example, <code class="text-[var(--text-secondary)]">#123</code> could
        link to issue #123 in your issue tracker.
      </p>
      ...
      <Show when={showCreate()}>
        <div class="p-3 bg-[var(--background-base)] rounded-[var(--radius-md)] border border-[var(--border-default)] space-y-3">
          ...
          <button
            ...
            disabled={!newPattern().trim() || !newUrl().trim()}
          >
            Add
          </button>
        </div>
            ...
            {(linkifier) => (
              <div class="flex items-center justify-between px-3 py-2.5 border-b border-[var(--border-default)] last:border-b-0">
                <div class="min-w-0">
                  ...
                  <div class="text-[10px] font-mono text-[var(--text-tertiary)] mt-0.5 truncate">
                    {linkifier.urlTemplate}
                  </div>
                </div>
                <button
                  class="text-[10px] text-[var(--status-error)] hover:underline shrink-0 ml-2"
                  onClick={() => handleRemove(linkifier.id)}
                >
                  Remove
                </button>
              </div>
