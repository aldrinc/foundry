...
export function SettingsMutedUsers() {
    const [mutedUsers, setMutedUsers] = createSignal([]);
    ...
    return (<div class="space-y-6">
      ...
      <Show when={mutedUsers().length > 0} fallback={<div class="text-center py-8">
        ...
        <div class="border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden">
          <For each={mutedUsers()}>
            {(user) => (<div class="flex items-center justify-between px-3 py-2 border-b border-[var(--border-default)] last:border-b-0">
                ...
                <button class="text-[10px] text-[var(--interactive-primary)] hover:underline shrink-0 ml-2" onClick={() => handleUnmute(user.id)}>
                  Unmute
                </button>
              </div>)}
          </For>
