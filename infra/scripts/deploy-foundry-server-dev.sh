...
ANTHROPIC_API_KEY="${FOUNDRY_ANTHROPIC_API_KEY:-${ANTHROPIC_API_KEY:-}}"
ANTHROPIC_API_BASE_URL="${FOUNDRY_ANTHROPIC_API_BASE_URL:-}"
ANTHROPIC_MODEL="${FOUNDRY_ANTHROPIC_MODEL:-}"
...
CODER_CONTAINER_NAME="${FOUNDRY_CODER_CONTAINER_NAME:-foundry-coder}"
CODER_TEMPLATE_NAME="${FOUNDRY_CODER_TEMPLATE_NAME:-foundry-hetzner-workspace}"
CODER_TEMPLATE_SOURCE_DIR="${FOUNDRY_CODER_TEMPLATE_SOURCE_DIR:-/opt/foundry/coder-templates/foundry-hetzner-workspace}"
CODER_INTERNAL_URL="${FOUNDRY_CODER_INTERNAL_URL:-http://127.0.0.1:7080}"
HCLOUD_TOKEN="${FOUNDRY_HCLOUD_TOKEN:-}"
WORKSPACE_PRIVATE_NETWORK_ID="${FOUNDRY_WORKSPACE_PRIVATE_NETWORK_ID:-}"
WORKSPACE_FIREWALL_IDS="${FOUNDRY_WORKSPACE_FIREWALL_IDS:-}"
WORKSPACE_SSH_KEY_IDS="${FOUNDRY_WORKSPACE_SSH_KEY_IDS:-}"
...
TEMPLATE_DEST="/opt/foundry/coder-templates/foundry-hetzner-workspace"
...
rsync -az --delete \
  -e "ssh -i ~/.ssh/hetzner_prod -o StrictHostKeyChecking=accept-new" \
  "${TEMPLATE_SRC}" "root@${HOST}:${TEMPLATE_DEST}/"
...
if [[ -f /etc/foundry/foundry-server.env ]]; then
  ...
  [[ -n "${ANTHROPIC_API_KEY}" ]] || ANTHROPIC_API_KEY="$(read_env_value FOUNDRY_ANTHROPIC_API_KEY)"
  [[ -n "${ANTHROPIC_API_BASE_URL}" ]] || ANTHROPIC_API_BASE_URL="$(read_env_value FOUNDRY_ANTHROPIC_API_BASE_URL)"
  [[ -n "${ANTHROPIC_MODEL}" ]] || ANTHROPIC_MODEL="$(read_env_value FOUNDRY_ANTHROPIC_MODEL)"
  ...
  [[ -n "${CODER_CONTAINER_NAME}" ]] || CODER_CONTAINER_NAME="$(read_env_value FOUNDRY_CODER_CONTAINER_NAME)"
  [[ -n "${CODER_TEMPLATE_NAME}" ]] || CODER_TEMPLATE_NAME="$(read_env_value FOUNDRY_CODER_TEMPLATE_NAME)"
  [[ -n "${CODER_TEMPLATE_SOURCE_DIR}" ]] || CODER_TEMPLATE_SOURCE_DIR="$(read_env_value FOUNDRY_CODER_TEMPLATE_SOURCE_DIR)"
  [[ -n "${CODER_INTERNAL_URL}" ]] || CODER_INTERNAL_URL="$(read_env_value FOUNDRY_CODER_INTERNAL_URL)"
  [[ -n "${HCLOUD_TOKEN}" ]] || HCLOUD_TOKEN="$(read_env_value FOUNDRY_HCLOUD_TOKEN)"
  [[ -n "${WORKSPACE_PRIVATE_NETWORK_ID}" ]] || WORKSPACE_PRIVATE_NETWORK_ID="$(read_env_value FOUNDRY_WORKSPACE_PRIVATE_NETWORK_ID)"
  [[ -n "${WORKSPACE_FIREWALL_IDS}" ]] || WORKSPACE_FIREWALL_IDS="$(read_env_value FOUNDRY_WORKSPACE_FIREWALL_IDS)"
  [[ -n "${WORKSPACE_SSH_KEY_IDS}" ]] || WORKSPACE_SSH_KEY_IDS="$(read_env_value FOUNDRY_WORKSPACE_SSH_KEY_IDS)"
...
if [[ -n "${ANTHROPIC_API_KEY}" ]]; then
  echo "FOUNDRY_ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY}" >> /etc/foundry/foundry-server.env
fi

if [[ -n "${ANTHROPIC_API_BASE_URL}" ]]; then
  echo "FOUNDRY_ANTHROPIC_API_BASE_URL=${ANTHROPIC_API_BASE_URL}" >> /etc/foundry/foundry-server.env
fi

if [[ -n "${ANTHROPIC_MODEL}" ]]; then
  echo "FOUNDRY_ANTHROPIC_MODEL=${ANTHROPIC_MODEL}" >> /etc/foundry/foundry-server.env
fi
