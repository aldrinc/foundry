...
if [[ $# -lt 2 ]]; then
  echo "usage: $0 <host> <workspace-name>" >&2
  exit 1
fi
...
ssh -i ~/.ssh/hetzner_prod -o StrictHostKeyChecking=accept-new "root@${HOST}" \
  "WORKSPACE_NAME='${WORKSPACE_NAME}' REPO_ID='${REPO_ID}' REPO_URL='${REPO_URL}' bash -s" <<'EOF'
set -euo pipefail
...
docker exec \
  ...
  sh -lc '
    ...
    /opt/coder delete "'"${WORKSPACE_NAME}"'" --yes >/dev/null 2>&1 || true
