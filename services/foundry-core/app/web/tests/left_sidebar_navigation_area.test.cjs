...
const {page_params} = zrequire("page_params");
...
run_test("narrowing", ({override_rewire}) => {
    ...
    filter = new Filter([{operator: "is", operand: "resolved"}]);
    left_sidebar_navigation_area.handle_narrow_activated(filter);
    assert.ok($(".top_left_archived_topics").hasClass("top-left-active-filter"));
...
run_test("restore_views_state defaults to collapsed", () => {
    page_params.is_spectator = false;
    const ls = localstorage();
    ls.remove("left_sidebar_views_state");
    ...
    assert.ok($views_label_container.hasClass("showing-condensed-navigation"));
    assert.ok(!$views_label_container.hasClass("showing-expanded-navigation"));
    assert.ok($views_label_icon.hasClass("rotate-icon-right"));
    assert.ok(!$views_label_icon.hasClass("rotate-icon-down"));
});

run_test("restore_views_state keeps expanded preference", () => {
    page_params.is_spectator = false;
    const ls = localstorage();
    ls.set("left_sidebar_views_state", "expanded");
