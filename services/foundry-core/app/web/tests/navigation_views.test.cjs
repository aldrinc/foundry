...
run_test("get_built_in_views", () => {
    ...
    const archived_topics_view = built_in_views.find((view) => view.fragment === "topics/is/resolved");
    assert.ok(archived_topics_view);
    assert.equal(
        archived_topics_view.name,
        built_in_views_meta_data.archived_topics.name,
    );
